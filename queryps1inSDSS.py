...
d = pd.read_csv('catalog/S82_release_ab_v3_2.csv',delim_whitespace=True,comment='#')
#print(d[['g_Nobs','g_mmu','g_msig']])
...
for i,row in d.iterrows():
    #print(i)
    if row['g_mmu'] > 19: continue
    if row['dec'] < -29: continue
    #if row['ra'] < 0: continue
    #if row['g_Nobs'] < 4: continue
    #if row['rarms'] > .01: continue
    if i > 5000: continue
    print(i,'out of',l)
    ...
    for filter in 'griz':
        ...
        for pcol in pcols:
            tab['SDSS_'+pcol] = row[pcol]
    print(tab)
    tables.append(tab)
bigtab = vstack(tables)
print(bigtab)
try:
    os.remove('newcatalog/PS1_in_SDSS.fits')
except:
    pass
bigtab.write('newcatalog/PS1_in_SDSS.fits')
