...
#d = pq.read_table('newcatalog/ubercal_star_mag_Zp_uv_pix_off_180_180_k_eff_outrej_p1000s1000p50s50p10s10p5s5_catalog_fid1.parquet')
d = pd.read_csv('newcatalog/SMALL_ZTF.DAT', delim_whitespace=True)
...
for i,row in d.iterrows():
    if row['ZTF_g'] > 19: continue
    if row['DEC'] < -29: continue
    print(i,'out of',l)

    ra = row['RA']
    dec = row['DEC']
    #radius = 0.6*0.000277778
    radius = 0.3*0.000277778
    ...
    for filter in 'gri':
        for pcol in pcols:
            tab['ZTF_'+pcol] = row[pcol]
    print(tab)
    tables.append(tab)
bigtab = vstack(tables)
print(bigtab)
try:
    os.remove('newcatalog/PS1_in_ZTF.fits')
except:
    pass
bigtab.write('newcatalog/PS1_in_ZTF.fits')
