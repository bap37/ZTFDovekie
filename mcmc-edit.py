...
def get_all_obsdfs(surveys, redo=False): #TODO - better checks on IRSA loading and such
    surveydfs = {}
    surveydfs_wext = {}
    for survey in surveys:
        ...
        if redo:
            ...
            obsdf = pd.read_csv('output_observed_apermags/%s_observed.csv'%obssurvmap[survey])
            surveydfs[survey] = obsdf
...
def getchi_forone(pars,surveydata,obsdfs,colorsurvab,surv1,surv2,colorfilta,colorfiltb,yfilt1,yfilt2,
    ...
    for shift in np.unique(df2x['shift'].values):
        ...
        for cat in np.unique(df2['standard_catagory']):
            ...
            reserr = datares*0+sigmadata/np.sqrt(len(datares))
